...
TrainImgLoader = DataLoader(train_dataset, args.batch_size, sampler=train_sampler, num_workers=8, drop_last=True)
TestImgLoader = DataLoader(test_dataset, args.test_batch_size, sampler=test_sampler, num_workers=4, drop_last=False)

kittiloadertest_sampler = torch.utils.data.distributed.DistributedSampler(kittiloadertest)
TestImgLoaderkitti = DataLoader(kittiloadertest, args.test_batch_size, sampler=kittiloadertest_sampler, num_workers=4, drop_last=False)
...
def train():
    ...
    for epoch_idx in range(start_epoch, args.epochs):
        ...
        # saving checkpoints
        if local_rank == 0 and (epoch_idx + 1) % args.save_freq == 0:
        ...
        if epoch_idx >= -1:
         ...
         for batch_idx, sample in enumerate(TestImgLoaderkitti):
            ...
            start_time = time.time()
            do_summary = global_step % args.summary_freq == 0
            loss, scalar_outputs, image_outputs = test_sample(sample, dataset='igarss', compute_metrics=do_summary)
            # loss, scalar_outputs, image_outputs = test_sample(sample, compute_metrics=do_summary)
            if do_summary:
                save_scalars(logger, 'test', scalar_outputs, global_step)
                save_images(logger, 'test', image_outputs, global_step)
...
def train_sample(sample, compute_metrics=False):
    ...
    # DDP
    imgL = imgL.to(local_rank)
    imgR = imgR.to(local_rank)
    disp_gt = disp_gt.to(local_rank)
    optimizer.zero_grad()

    disp_ests = model(imgL, imgR)
    if 'igarss' in args.dataset:
        mask = (disp_gt != -999.0) & (disp_gt < args.maxdisp) & (disp_gt > 0)
        disp_gt[~mask] = 0.0
    else:
...
# bug
# RuntimeError: Expected to have finished reduction in the prior iteration before starting a new one. This error indicates that your module has parameters that were not used in producing loss. You can enable unused parameter detection by (1) passing the keyword argument `find_unused_parameters=True` to `torch.nn.parallel.DistributedDataParallel`; (2) making sure all `forward` function outputs participate in calculating loss. If you already have done the above two steps$ then the distributed data parallel module wasn't able to locate the output tensors in the return value of your module's `forward` function. Please include the loss function and the structure of the return value of `forward` of your mo$ule when reporting this issue (e.g. list, dict, iterable).    
